...
<?php
	if (isset($_GET['id'])){
		...
		$booking = $conn->query("SELECT * FROM bookings WHERE hotel_name ='$singleRoom->hotel_name' AND room_name = '$singleRoom->name'");
		$booking->execute();
		...
		if (isset($_POST['submit'])) {
			...
			$check_out_old = date_create($singleBooking->check_out);
			$email = $_POST['email'];
			...
			if (empty($check_in) || empty($check_out) || empty($email) || empty($phone_number) || empty($full_name)) {
				...
				$check_out_old_str = $check_out_old->format('Y-m-d');
				
				$today = date("Y-m-d");
				$tomorrow = date("Y-m-d", strtotime('+1 day'));
		
				// Kiểm tra logic ngày
				if (strtotime($check_in_str) < strtotime($tomorrow)) {
					echo "<script> alert('Check-in date must be from tomorrow onwards')</script>";
				} 
				elseif (strtotime($check_out_str) <= strtotime($check_in_str)) {
					echo "<script> alert('Check-out date must be after the check-in date')</script>";
				} 
				elseif (strtotime($check_in_str) <= strtotime($check_out_old_str)){
					...
					$booking = $conn->prepare("INSERT INTO bookings (check_in, check_out, email, phone_number, full_name,
						...
						VALUES (:check_in, :check_out, :email, :phone_number, :full_name, :hotel_name, :room_name,
      ...
      <div class="container">
        <div class="row no-gutters slider-text js-fullheight align-items-center justify-content-start" data-scrollax-parent="true">
          <div class="col-md-7 ftco-animate">
          	<h2 class="subheading">Welcome to Booking Aura</h2>
	    		...
	    		<div class="col-lg-4">
					<form action="room-single.php?id=<?php echo $id?>" method = "POST"class="appointment-form" style="margin-top: -568px;">
						...
						<div class="row">
							...
							<?php else : ?>
								<div class="form-group">
								<p class = "text-center">Login in order to book a room</p>
								<a href="<?php echo APPURL; ?>/auth/login.php" class="btn btn-primary py-3 px-4">Login</a>
								</div>
							<?php endif; ?>
						</div>
				</form>
